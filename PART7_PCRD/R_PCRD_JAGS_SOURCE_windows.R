...
generate.z.psi <- function(y,T2,first.capture=FALSE,z.priors = list(phi.beta=c(shape1=30,shape2=5),g1.beta=c(shape1=20,shape2=20),g2.beta=c(shape1=20,shape2=20), pd.beta=c(shape1=12,shape2=65)),exclude_=1, dead_=2,out_=3,in_=4, nstates = 4){
    #exclude_=1; dead_=2;out_=3;in_=4; nstates = 4           
     ...
     for(t_ in 1:T){
         t.mat[,exclude_,t_]<-c(1-psi[t_],0,psi[t_]*(1-lambda[t_]),psi[t_]*lambda[t_])[c(exclude_,dead_,out_,in_)]
         t.mat[,dead_,t_]<-c(0,1,0,0)[c(exclude_,dead_,out_,in_)]
         t.mat[,out_,t_]<-c(0,1-phi[t_],g1[t_]*phi[t_],(1-g1[t_])*phi[t_])[c(exclude_,dead_,out_,in_)]
         t.mat[,in_,t_]<-c(0,1-phi[t_],g2[t_]*phi[t_],(1-g2[t_])*phi[t_])[c(exclude_,dead_,out_,in_)]
     }
