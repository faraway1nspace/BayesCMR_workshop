...
post.samp <- coda.samples(m, variable.names=c("mu","sigma"),n.iter=niter,thin=thin)
...
model2.txt <- '
model{
 # Normal priors regression coefficients
 ...
 for(i in 1:length(y)){
   ...
   y[i] ~ dnorm(mu[i],tau) T(0,) # truncated normal
 }
 sigma <- pow(tau,-0.5)
}'
