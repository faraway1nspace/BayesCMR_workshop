...
- biased (towards the prior)\footnote{A Bayesian would claim that given a prior, it would be irrational to believe in anything other than the posterior.}
- language dependence
* History
...
* History - Frequentism's Ascendency
frequentists:
- reject probabilistic confirmation of models \footnote{Some use the AICc for model confirmation}
...
the mean counts of heads in the long-run
* History - Hypotheses
** _Frequentism_
:PROPERTIES:
:BEAMER_env: block
...
- /probability/ of a /probability/: $p(\theta\vert y)$
...
- $p(y\!=\!\text{Head}\vert \theta) = \text{Bern}(y;\theta)\rightarrow\text{What we now call a likelihood}$
...
- *After data*: $p(\theta\vert Y)$ (the /posterior/ probability distribution)
* Bayes Theorem
- conditional probability: want a probability distribution $p(\theta\vert y)$ conditional on observed data $y$ -> need a likelihood $f(Y\vert \theta)$ and prior probability distribution $p(\theta)$.
\begin{equation}
\begin{aligned}
