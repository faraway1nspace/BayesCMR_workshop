...
jags.txt <- "model{
  ...
  pr.gamma2 <- 0.01
  # priors
  mu ~ dnorm(pr.mu, pr.tau) T(0,)
  sigma ~ dgamma(pr.gamma1,pr.gamma2)
  # transform tau (precision) to a root(variance)
  tau <- 1/(sigma^2)
  # likelihood
  for(i in 1:N){
     y[i] ~ dnorm(mu,tau) T(0,)
  }
}"
cat(jags.txt, file="/tmp/jagsmodel.jag")
jags.data <- list(y = y,
                  N = length(y))
...
m1 <- jags.model(file = "/tmp/jagsmodel.jag",
           data = jags.data,
           n.chains = n.chains,
           n.adapt = n.adapt) # adaption phase
update(m1, 10000) # burn-in phase for the MCMC sampler
# get samples from the posterior distributions
post.samp <- coda.samples(model = m1,
                          variable.names = c("mu","tau","sigma"),
                          n.iter = 100000,
                          thin = 100)

y.pred <- numeric(nrow(post.samp[[1]]))
for(i in 1:nrow(post.samp[[1]])){
    y.pred[i] <- rnorm(n=1, mean = post.samp[[1]][i,"mu"], sd=post.samp[[1]][i,"sigma"])
}
...
hist(y.pred)
                                        ...
                                        # plot the prior
plot(density(post.samp[[1]][,"mu"]),col="red")
lines(density(rnorm(10000,167,30)))
# add the posterior
